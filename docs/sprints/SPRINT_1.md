...
**Definition of done:**
- [ ] All routes in [Acceptance criteria](#acceptance-criteria) below behave correctly
- [ ] Tests cover happy paths and key failure modes (>70% coverage on `auth/` packages)
- [ ] Login + register UI exists, looks decent (using shadcn-svelte, Kliniq Emerald theme)
- [ ] Passkey registration + assertion works end-to-end in Chrome on macOS/Windows
- [ ] OWASP ASVS L2 auth checklist (below) all green
- [ ] CI green
...
- [ ] V2.1.1 Passwords ≥ 12 chars
- [ ] V2.1.2 Allow long passphrases (max 128 chars enforced)
- [ ] V2.1.7 Block top-1000 common passwords (use list from CrackStation or HIBP API)
- [ ] V2.2.1 Anti-automation (rate limiting on auth endpoints)
- [ ] V2.2.3 Notify user of authentications from new devices (V2 — defer)
- [ ] V2.4.1 Argon2id with parameters from ADR-010
- [ ] V2.5.4 Don't reveal whether user exists on register/forgot
- [ ] V2.7.1 OOB tokens (email tokens) at least 20 bits entropy → we use 256
- [ ] V2.7.2 OOB tokens single-use (consumed_at)
- [ ] V2.7.3 OOB tokens time-limited (15min reset, 24h verify)
- [ ] V3.2.1 Sessions invalidated on logout
- [ ] V3.2.2 Session ID generated by framework (Spring Session) using CSPRNG
- [ ] V3.4.1 Cookies have Secure + HttpOnly + SameSite=Lax + __Host- prefix
- [ ] V3.5.1 Logout terminates session everywhere
- [ ] V3.7.1 Re-auth before sensitive changes (password change requires current password)
- [ ] V13.1.1 CSRF tokens on state-changing requests
- [ ] V13.2.6 Rate limiting + exponential backoff on login failures
...
After done, fill out at bottom of this file:
...
Audit log review (run query, sample 10 events): looks consistent? yes/no
Security review: re-read OWASP ASVS list, anything I cheated on? ___
```

→ Then move to Sprint 2 planning.
